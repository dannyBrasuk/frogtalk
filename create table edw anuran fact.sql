...
CREATE TABLE edw.anuran_fact  ( 
    ...
    observation_type_id INT NOT NULL,       --heard, seen alive, found dead
    ...
    site_characteristics_fk INT NULL,                        --such as land use code.  (A type 4 on the site dim)
    observed_site_weather_conditions_fk INT NULL,             --condition of site per observer (as fixed attribute categories)

    weather_station_nearest_fk  INT NULL,                    --official weather measurements station

    --judgement call on whether to collaspe weather values into a dim, or not.
    ...
    --these two could be collasped into a single dimension, for simplicity.
